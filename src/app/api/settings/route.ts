...
import { DEFAULT_ACCOUNTING_FOLLOW_UP_MESSAGE, DEFAULT_ACCOUNTING_MESSAGE, DEFAULT_SETTINGS, DEFAULT_WEBSITE_FOLLOW_UP_MESSAGE, DEFAULT_WEBSITE_MESSAGE } from "@/data/defaults";
...
export async function GET() {
  try {
    ...
    const [{ data: settings }, { data: templates }] = await Promise.all([
      supabase.from("app_settings").select("results_per_search,daily_contact_goal,website_sectors,accounting_sectors,website_follow_up_message,accounting_follow_up_message,first_follow_up_days,final_follow_up_days,max_follow_ups").eq("user_id", user.id).maybeSingle(),
