  ...
  return (
    <section>
      ...
      <div className="search-filters" aria-label="Arama filtreleri">
        ...
        <label><span><BriefcaseBusiness size={15} />Meslek / sektör</span><select value={sectorFilter} onChange={(event) => setSectorFilter(event.target.value)}><option value="">Tüm aktif sektörler · başarı öncelikli</option>{availableSectors.map((sector) => <option key={sector} value={sector}>{sector}</option>)}</select></label>
        ...
        <p>{presenceFilter === "instagram" ? "Faal, geçerli cep telefonlu, Instagram bağlantılı ve bağımsız sitesi olmayan işletmeler aranır. Son iki yılda açıldığı doğrulananlar önce gelir; tarih kaydı olmayan uygun adaylar artık kaybedilmez." : provinceFilter && sectorFilter ? "Seçilen şehir ve meslekte faal görünen, sektörle eşleşen ve BTK planına uygun mobil numarası olan işletmeler aranır. WhatsApp işareti ve yeni açılış tarihi bulunanlar önce gösterilir." : qualityFilter === "selective" ? "Seçici modda geçerli mobilin yanında çalışma saati veya yakın tarihli doğrulama gibi faal işletme sinyali de zorunludur." : qualityFilter === "broad" ? "Geniş mod daha fazla sektör eşleşmesi getirir; açıkça eski açılış tarihli, kapanmış veya geçersiz numaralı kayıtları yine eler." : "Dengeli mod 81 il ile tüm aktif sektörleri sırayla tarar ve sonuçları farklı şehir/sektörlere dağıtır. İlgilenme veya demo sonucu alınan alanlar önce denenir; bağımsız sitesi olan, kapanmış, eski açılışlı ya da geçersiz mobil numaralı kayıtlar elenir."}</p>
      </div>
